...
class Topmenu extends \Magento\Framework\View\Element\Template
{
    ...
    public function getSubmenuItemsHtml($children, $level = 1, $max_level = 0, $column_width=12, $menu_type = 'fullwidth', $columns = null, $parent_cat_id = null)
    {
        $html = '';
        ...
        if(!$max_level || ($max_level && $max_level == 0) || ($max_level && $max_level > 0 && $max_level-1 >= $level)) {
            ...
            $html = '<ul class="submenu '.$column_class.'">';
            foreach($children as $child) {

                $parent_cat_id_child = $child->getId();
                $cat_model = $this->getCategoryModel($child->getId());

                $sw_menu_hide_item = $cat_model->getData('sw_menu_hide_item');

                $_objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                $object_manager = $_objectManager->create('Magento\Catalog\Model\Category')->load($parent_cat_id);
                ...
                if (!$sw_menu_hide_item) {
                    ...
                    if(count($sub_children) > 0) {
                        $html .= '<div class="open-children-toggle test1"></div>';
                    }
...
//                    $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($object_manager).'">';
                    if ($level > 1 && $sw_menu_icon_img)
                    ...
                    $html .= '</span></a>';
                    if(count($sub_children) > 0) {
                        $html .= $this->getSubmenuItemsHtml($sub_children, $level+1, $max_level, $column_width, $menu_type,null ,$parent_cat_id_child);
                    }
                    $html .= '</li>';
                }
            }
...
//            $object_manager = $_objectManager->create('Magento\Catalog\Model\Category')->load($parent_cat_id);
            ...
            if($level == 1) {
                $html .= '<li class="ui-menu-item "><a class="menu-thumb-link" href="' . $this->_categoryHelper->getCategoryUrl($object_manager) . '">Shop All</a>';
            }
        ...
        foreach($categories as $category) {
            ...
            if(!$sw_menu_hide_item) {
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content)))
                    $item_class .= 'parent ';
                $html .= '<li class="ui-menu-item '.$category->getName().' '.$item_class.$sw_menu_float_type.'">';
                if(count($children) > 0) {
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content))) 
                    {
                        $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($category).'" class="level-top test1" id="'.$custom_id++.'">';
                        } 
                    else 
                    {
                        $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($category).'" class="level-top" id="'.$custom_id++.'">';
                    }
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_top_content || $menu_left_content || $menu_right_content || $menu_bottom_content))) {
                    ...
                    if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_left_content || $menu_right_content))) {
                        ...
                        $html .= $this->getSubmenuItemsHtml($children, 1, $max_level, 12-$menu_left_width-$menu_right_width, $menu_type, $sw_menu_cat_columns, $parent_cat_id);
