...
class Product extends AbstractBlock
{
    ...
    {
        foreach ($this->configHelper->getCustomAttributes() as $customAttribute) {
            $customAttribute = trim($customAttribute);
            if ($product->getData($customAttribute)) {
                $result[$customAttribute] = $product->getAttributeText($customAttribute) ?
                ...
                $result[self::MPN_IDENTIFIER] = $product->getData(self::SKU_IDENTIFIER);
            }
        }
    }
    ...
    private function getProductDescription($product)
    {
        ...
        switch ($this->configHelper->getProductDescriptionMode()) {
            ...
            case DescriptionSource::META_DESCRIPTION:
                $description =  $this->getMetaData($product, 'meta_description') ?: $this->pageConfig->getDescription();
                break;
        }
