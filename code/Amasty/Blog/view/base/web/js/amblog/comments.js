...
define([
    ...
    $.widget('mage.amblogComments', {
        ...
        _create: function () {
            ...
            $(document).on('change', self.options.gdpr, function () {
                if ($(this).is(':checked')) {
                    $(this).closest(self.options.form).find('.amblog-customer').removeClass('-hidden');
                } else {
                    $(this).closest(self.options.form).find('.amblog-customer').addClass('-hidden');
                }
            });
