...
class MassExport extends \Magento\Sales\Controller\Adminhtml\Order\AbstractMassAction
{
    ...
    */
	...
	protected function getOrderItemValues($item, $order, $itemInc=1) 
    {
        ...
        //load amasty giftcard factory by quote
        $giftCardQuoteCollection = $this->quoteGiftCardCollectionFactory->create()
                                    ->getGiftCardsByQuoteId($order->getQuoteId());
        $giftCode = "";
        $giftAmount = "";
        foreach ($giftCardQuoteCollection as $giftCard) {
            $giftCode = $giftCard->getCode(); // gift card code
            $giftAmount = $giftCard->getGiftAmount(); // gift card amount
        }
        ...
        return array(
            ...
            $order->getCoupondiscountCode(),
            $order->getCoupondiscountTotal(),
            $giftCode,
            $giftAmount
        );
