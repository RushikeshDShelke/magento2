...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        if ($this->sessionManager->getPagesViewedByCustomer()) {
            $this->sessionManager->setPagesViewedByCustomer($this->sessionManager->getPagesViewedByCustomer() +1);
        } else {
            $this->sessionManager->setPagesViewedByCustomer(1);
        }

        return $this->sessionManager->getPagesViewedByCustomer();
    }
    ...
    {
        $displayedPopup = (!empty($this->sessionManager->getDisplayedPopups()))?
                            $this->sessionManager->getDisplayedPopups(): [0];

        if (!in_array($id, $displayedPopup)) {
            $displayedPopup[] = $id;
        }

        $this->sessionManager->setDisplayedPopups($displayedPopup);
    }
    ...
    {
        $displayed = true;
        $listPopupDisplayed = (!empty($this->sessionManager->getDisplayedPopups()))?
                                $this->sessionManager->getDisplayedPopups(): [0];
